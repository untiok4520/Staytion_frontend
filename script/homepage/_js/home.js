...
function showSuggestions(keyword = "") {
  ...
  if (keyword.trim() === "") {
    ...
    // 發送 API 請求
    fetch(
      `http://localhost:8080/api/locations/search?keyword=${encodeURIComponent(
        keyword
      )}`
    )
...
async function performSearch() {
  const locationName =
    document.getElementById("destinationInput").value || "台北市";
...
// 取得容器和箭頭按鈕
const recommendationCarousel = document.getElementById(
  "recommendation-carousel"
);
const nextRecommendationBtn = document.getElementById(
  "scroll-next-recommendation"
);
const prevRecommendationBtn = document.getElementById(
  "scroll-prev-recommendation"
);
    ...
    for (const hotel of hotels) {
      ...
      card.innerHTML = `
                <a href="#" class="text-decoration-none text-dark d-block position-relative">
                    ...
                    <button class="btn btn-light heart-btn position-absolute top-0 end-0 m-2 rounded-circle shadow-sm" data-hotel-id="${
                      hotel.hotelId
                    }">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text text-muted small"><i class="bi bi-geo-alt me-1"></i>${
                          hotel.districtName
                        }, ${hotel.cityName}</p>
                        <p class="mb-1">
                            <span class="badge rate">${hotel.averageRating.toFixed(
                              1
                            )}</span>
      ...
      if (userId) {
        ...
        const isFavoriteResponse = await fetch(
          `http://localhost:8080/api/favorites/check?userId=${userId}&hotelId=${hotel.hotelId}`
        );
...
document.addEventListener("click", function (e) {
  ...
  if (link) {
    e.preventDefault();
    const hotelId = link.closest(".card").querySelector("button.heart-btn")
      .dataset.hotelId;
...
function removeFavorite(userId, hotelId) {
  ...
  return fetch(
    `http://localhost:8080/api/favorites?userId=${userId}&hotelId=${hotelId}`,
    {
      method: "DELETE",
      headers: {
